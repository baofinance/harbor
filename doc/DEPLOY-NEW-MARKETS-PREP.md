...
- **Blockscout (MegaETH):** After verifying on mega.etherscan.io, verify the same deployment on Blockscout so contracts show on https://megaeth.blockscout.com. Use `script/verify-megaeth-blockscout` (same state file as `script/verify-megaeth`). No API key required. Run: `script/verify-megaeth-blockscout --state deployments/megaeth/<salt>.state.json` for implementations; add `--verify-proxies` and set `UUPS_PROXY_DEPLOY_STUB_ADDRESS` to verify proxy contracts. Reference: harbor-price-aggregators `script/megaeth/verify/verify-megaeth-v4-oracles.sh` and `verify-megaeth-proxies.sh`.
