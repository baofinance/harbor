...
Sail tokens that maintain a **constant leverage ratio** (e.g., 2x, 3x, 4x) through active rebalancing.

**Mathematical form:**
```
Leverage(C) = L_target  (constant)
Requires: Rebalance when |Leverage_actual - L_target| > threshold
```

**Mechanism:** When collateral price moves:
1. Calculate actual leverage: L_actual = C / (C - A)
2. If L_actual drifts outside band (e.g., 2x ± 0.2x):
   - Adjust exposure by minting/redeeming anchored tokens
   - Restore leverage to 2x

**Example:** 2x sail token, C = $100, A = $50 (L = 2x)
- C → $110: L → 110/60 = 1.83x (below 2x)
- Rebalance: Mint $5 anchored (A → $55), sail still $55, L = 110/55 = 2x ✓
...
2. **No gamma drag:** Rebalancing removes convexity, linear returns
...
**1. Rebalancing drag (higher than asymmetric)**

Fixed leverage requires **more frequent rebalancing** to stay within tight bands.

**Mathematical impact:**
```
Rebalance frequency ∝ (σ / band_width)²
```

For 2x token with ±10% band (1.8x to 2.2x):
```
Band in C-space: ±5% (tighter than asymmetric leverage)
Expected rebalances/year ≈ (0.8 / 0.1)² × 0.8 = 51 rebalances
Cost = 51 × 0.3% = 15.3% annual drag
```

**Comparison:**
- Asymmetric leverage (±20% band): 3.2 rebalances/year, 0.96% drag
- Fixed leverage (±10% band): 51 rebalances/year, 15.3% drag

**Risk:** Significantly higher costs in volatile markets, can exceed funding costs of perps.

**2. Volatility decay (path dependency)**

Even with perfect rebalancing, fixed leverage loses value in choppy markets.

**Example:** C = $100 (day 0)
- Day 1: C → $110 (+10%), 2x token → +20%, value = $120
- Day 2: C → $100 (-9.1%), 2x token → -18.2%, value = $98.2
...
This is extreme, but illustrates the path dependency.

**Risk:** Severe underperformance in high-volatility, mean-reverting markets. Can lose money even if underlying is flat.

**3. Rebalancing latency**

Rebalancing is not instantaneous:
1. Oracle detects band breach
2. Keeper submits transaction
3. DEX swap settles (if using CowSwap: minutes to hours)

During this window, leverage continues to drift.

**Impact:** If C drops 10% and rebalancing takes 1 hour, and C drops another 5% during that hour, actual leverage may exceed 3x briefly (should be 2x).

**Risk:** Temporary over-leverage, potential for larger losses than expected.

**4. Capacity constraints**

Fixed leverage minting requires **additional collateral** from stability pool (or equivalent).

**Example:** User mints 2x token with $100 collateral
- Need $100 backing from stability pool (USDC or equivalent)
- Total: $200 effective collateral for $100 notional

**Constraint:** Cannot mint more 2x tokens than stability pool has USDC available.

**Risk:** Capped supply, potential shortages during high demand (bull markets).

#### Summary: User Cost/Benefit Analysis

| Benefit | Cost/Risk | Net |
|---------|-----------|-----|
| Predictable 2x exposure | High rebalancing drag (-15% pa) | **-15% drag** |
| No gamma (linear) | Volatility decay (-128% in extreme cases) | Severe in choppy markets |
| Institutional clarity | Capacity constraints | Limited supply |
| Comparable to leveraged ETFs | Rebalancing latency (temporary over-leverage) | Manageable with tight bands |

**Net assessment:** Useful for short-term tactical plays, poor for long-term holds in volatile markets. Better suited for lower-volatility assets (e.g., gold, stablecoins).

### Required Infrastructure

#### Critical (Does Not Exist Yet)

1. **Dual-asset stability pool:** Must hold anchored + equivalent (USDC/DAI)
   - Anchored tokens for normal deposits
   - USDC for funding leveraged token mints
   - Complex accounting: Track which USDC is "reserved" for leverage capacity

2. **Frequent rebalancing system:**
   - Keeper network that monitors leverage bands (every block)
   - Gas-efficient rebalancing (batch multiple users)
   - MEV protection (CowSwap or Flashbots)

3. **Dynamic fee structure:**
   - Mint fees increase as capacity (USDC in pool) depletes
   - Example: 0.5% at >50% capacity, 5% at <10% capacity
   - Prevents capacity exhaustion

#### Desirable Additions

4. **Capacity dashboard:** Real-time display of available leverage capacity
5. **Multi-tier leverage:** 2x, 3x, 4x tokens (different bands for each)
6. **Circuit breakers:** Pause minting during extreme volatility (>10% moves in 1 hour)

### Implementation Complexity

**High complexity:**
- Dual-asset stability pool: Significant contract changes
- Frequent rebalancing: High gas costs, keeper infrastructure
- Capacity management: Dynamic fees, allocation logic
...
**2. Funding rate requirements**

Without a balancing mechanism, demand for shorts vs longs will be imbalanced.

**Problem:** If everyone wants shorts (bear market), who provides the long side?

**Solutions:**
a. **Funding rate:** Shorts pay longs (or vice versa) to balance supply
   - Adds complexity, periodic payments
   - Breaks Harbor's "no funding" advantage

b. **Capacity limits:** Cap short token supply to equal long supply
   - Simpler, but limits utility
   - May not satisfy demand during bear markets

c. **Synthetic shorts via anchored tokens:** Holding anchored = short exposure to collateral volatility
   - Elegant, but not true leveraged short
   - Doesn't provide 2x/3x inverse moves

**Risk:** Either complexity (funding) or limited utility (caps).
...
- **S_short:** Short sail (if implemented)

**Synthetic positions:**

### 7.1: Synthetic Collateral Hold

**Construction:** Long sail (S) + Anchored (A)
...
**Result:** Equivalent to holding collateral directly, but decomposed.

**Use case:** None (just illustrating equivalence).

---

### 7.2: Leveraged Collateral (2x)

**Construction:** Long sail (S) + Borrow anchored (A) + Buy more sail
...
If A = 0.4C (typical ratio):
```
= 2C - 1.2C = 0.8C
```

Wait, this doesn't work cleanly. Actual 2x requires fixed leverage facet.

**Better construction:** Just mint Fixed Leverage 2x sail token (Facet 2).

---

### 7.3: Protected Long (Covered Call Analog)

**Construction:** Hold collateral (C) + Sell sail (S)

**Payoff:**
- You keep anchored tokens (A)
- Buyer of sail gets leveraged upside
- You cap upside at A but protected downside (collateral still yours)

**Result:** Similar to covered call (cap upside, keep premium).

**Use case:**
- Collateral holder who wants income + downside protection
- Bear market: Sell sail to others, collect premium (mint fee)

**Example:** C = $100, sell sail at S = $60 (A = $40)
- C → $120: You keep $100 collateral + $40 anchored, buyer gets $80 sail
  - Your return: $140 total (vs $120 if held collateral) — NO, this doesn't work because you sold sail, so buyer has claim

Actually, "selling sail" in Harbor means minting it, not selling existing. Let me reconsider.

**Actual mechanism:** Mint sail + anchored from your collateral, sell sail, keep anchored.
- You: $100 collateral → mint $40 anchored + $60 sail → sell sail for $60 → hold $40 anchored + $60 cash
- Total: $100 cash (equivalent to original), but if C rises, you don't participate (sold sail)

**This is just exiting the position, not a covered call.**

**Real covered call analog would require:**
- Ability to "short" sail tokens (lend them out)
- Earn lending rate (like covered call premium)
- Harbor doesn't have lending markets yet

---

### 7.4: Delta-Neutral Position

**Construction:** Long sail (S) + Short collateral perp (on CEX) with delta = leverage ratio

**Example:** C = $100, A = $40, S = $60, leverage = 1.67x
- Long 1 unit sail (delta = 1.67)
- Short 1.67 units wstETH perp
- Net delta = 0

**Payoff:**
- C → $110: S → $70 (+$10), Perp → -$16.7 (short loses)
  - Net: +$10 - $16.7 = -$6.7

Wait, this should be zero if delta-neutral. Let me recalculate.

**Corrected:**
- Long 1 unit sail (delta = 1.67 per $1 collateral move)
- Short 1.67 units collateral
- C → $110 (+$10 per unit):
  - Sail gains: 1.67 × $10 = $16.7
  - Short loses: 1.67 × $10 = $16.7
  - Net: $0 ✓

**But gamma is not hedged:**
- As C changes, sail's delta changes (gamma effect)
- Need to continuously rebalance short position

**Use case:**
- Earn from gamma (buy low delta, sell high delta)
- Earn from theta (rebalancing costs → profit if timed right)
- Arbitrage funding rates (if perp funding is positive, earn from short side)

**Advanced strategy:** Requires active management, suitable for sophisticated traders.

---

### 7.5: Stablecoin Yield Farming (Principal Protected)
...
- Sail + short perp = delta-neutral (no price risk if managed)
- Net: Earn stablecoin yield (5%) with no collateral price exposure

**Risk:** Rebalancing costs for delta hedge, potential for imperfect hedge.
...
|----------|--------------|---------|------------|
| Synthetic collateral | S + A | None (just equivalent) | Low |
| Leveraged collateral | Fixed leverage facet | Clean 2x/3x/4x | Medium (requires Facet 2) |
| Protected long | C + sell sail (need lending) | Cap upside, earn premium | High (requires lending market) |
...
**Purpose:** Enable fixed leverage facets (2x, 3x) and short leverage by providing external stablecoin liquidity.

#### What Users Get

- **Stability pool depositors:** Can deposit USDC (not just anchored tokens), earn yield on USDC
- **Leverage token minters:** Can mint 2x/3x tokens (stability pool provides USDC for additional collateral)
- **Short sail minters:** Can mint shorts (stability pool swaps wstETH → USDC as collateral)
...
3. When leverage token mints: Pool swaps anchored for USDC (or provides USDC directly)
...
- No USDC left for leverage token capacity
- System breaks (can't mint new leveraged tokens)
...
| **2. Fixed leverage** | Medium | High | High | High | **Very High** |
| **3. Short leverage** | High | Very High | High | Very High | **Extreme** |
| **4. Symmetric** | Same as Fixed | Same as Fixed | Same as Fixed | Same as Fixed | **Very High** |
...
**Smart contracts:** 3/5
- Dual-asset stability pool (anchored + USDC) = major refactor
...
- Optimal band widths (trade-off: tighter band = less drag but more frequent rebalancing)
- Fee curves (capacity-based)

**Audit scope:** 4/5
- Dual-asset pool is new attack surface
- Rebalancing at high frequency increases edge cases

**Recommendation:** **High complexity, 9-12 month timeline.** Only pursue if user demand is validated (survey, whitepaper interest).
...
**Goal:** Add unique features (tiered risk) that competitors don't have.

**Implementations:**

4. **Tiered risk sail tokens (Facet 5):**
...
**Decision point:** After 12 months, measure adoption of tiers. If senior + junior combined >$50M TVL, consider Phase 3.

---

### Phase 3: Advanced Features (Year 2+)

**Goal:** Explore complex facets (fixed leverage, shorts) if demand exists.

**Implementations:**

7. **Fixed leverage (Facet 2):**
   - Dual-asset stability pool (anchored + USDC)
   - 2x/3x fixed leverage tokens
   - Dynamic fee curves

   **Effort:** 9-12 months
   **Risk:** High

   **Prerequisites:** Volatility < 50% for 3 months, clear user demand (surveys/polls)

8. **Short leverage (Facet 3):**
   - Funding rate system
   - USDC-backed shorts
   - Inverse leveraged tokens

   **Effort:** 12-18 months
   **Risk:** Very High

   **Prerequisites:** Phase 1+2 successful, $100M+ TVL, experienced team

**Deliverables:**
- Fixed leverage tokens (if volatility permits)
- Short tokens (if user demand exists)
...
- Redundant with Fixed Leverage (same implementation)
- No additional user value

**Recommendation:** Skip entirely, treat as alias for Fixed Leverage in documentation.
...
| Fixed leverage (2x/3x) | Medium | Very High | High | Phase 3 (conditional) | Year 2+ |
| Short leverage (inverse) | Medium | Extreme | Very High | Phase 3 (conditional) | Year 2+ |
| Symmetric leverage | Low (redundant) | Same as Fixed | Same as Fixed | Rejected | N/A |
...
**Phase 2 (12 months):**
- $50M+ combined TVL (senior + junior)
- Aave/Compound listing (sail as collateral)
- <0.5% rebalancing drag annually
