# Variable Leveraged Sail Token (Current Harbor)
...
### 1.2 Behavior Characteristics

**Sail value response:**

```
ΔS ≈ ΔC (for price changes)

Linear in collateral - sail value changes 1:1 with collateral
```

**Leverage drift:**

```
Price increases → L decreases (de-leveraging when winning)
Price decreases → L increases (re-leveraging when losing)
```

**Negative gamma effect:** Sail token returns experience:

- Decelerating gains when winning (leverage drops from initial level)
- Accelerating losses when losing (leverage rises above initial level)

This creates path dependency - different price paths yield different returns even with same endpoints.

**Reference:** See [0-anchor-sail-mathematics.md](0-anchor-sail-mathematics.md) Section 4 for detailed operation effects and Section 4.7 for TVL scaling analysis.

### 1.3 Visual Analysis

**Leverage Drift with Price Changes**

![Leverage Decreases as Collateral Increases](charts/leverage-decreases-as-collateral-increases.png)

_Note: As collateral rises, leverage decreases. As collateral falls, leverage increases. This creates path-dependent returns._

**Gamma Analysis: Acceleration Near Wipeout**

![Leverage Gamma Variable Sail](charts/leverage-gamma-variable-sail.png)

_Note: Gamma represents acceleration of leverage changes. High gamma near wipeout (low C) means leverage changes accelerate during drawdowns._

**Operation Impact Scales with TVL**

![Mint Anchored TVL Impact](charts/mint-anchored-tvl-impact.png)

_Note: A 10% mint increases leverage by 10% regardless of TVL size. At $100M TVL, a $1M operation is only 1% (minimal impact). Larger protocols are more stable._
