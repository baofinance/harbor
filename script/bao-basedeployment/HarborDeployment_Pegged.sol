...
/// @dev Config context (baoFactory, owner, systemSalt) comes from ConfigProtocol via FactoryDeployer.
abstract contract HarborDeployment_Pegged is FactoryDeployer {
    ...
    /// @dev baoFactory(), owner(), systemSalt() come from ConfigProtocol inheritance.
    /// @param pegConfig Configuration for this peg (ConfigPeg_ETH, ConfigPeg_BTC, etc.).
    /// @return deployment Deployment records to be saved to state by caller.
    function deployPeggedToken(ConfigPeg pegConfig) internal returns (PeggedTokenDeployment memory deployment) {
        ...
        // Compute CREATE3 salt - systemSalt() from ConfigProtocol
        ...
        // Prepare initialization data - owner() from ConfigProtocol
        bytes memory initData = abi.encodeCall(MintableBurnableERC20_v1.initialize, (owner(), tokenName, tokenSymbol));

        // Deploy proxy via CREATE3 - baoFactory() from ConfigProtocol
