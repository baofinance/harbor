# StabilityPool v2 Upgrade Verification
...
The `run-upgrade-test-StabilityPool_v2` script orchestrates the full workflow
(fork block: 24433566). It prompts you to start and stop anvil manually between steps:

```bash
script/test/run-upgrade-test-StabilityPool_v2
```
...
```bash
# Only test BTC pools
POOL_FILTER=BTC script/test/run-upgrade-test-StabilityPool_v2

# Less verbose forge output
FORGE_VERBOSITY=-vv script/test/run-upgrade-test-StabilityPool_v2
```
...
```bash
script/anvil --block 24433566  # just before the SP v2 upgrade
```
