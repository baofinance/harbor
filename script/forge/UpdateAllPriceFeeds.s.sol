...
contract UpdateAllPriceFeeds is Script {
    function run() external {
        ...
        (bool success1, ) = stethUsdFeed.call(
            abi.encodeWithSignature("setLatestAnswer(int256)", stethUsdPrice)
        );
        require(success1, "Failed to update stETH/USD feed");

        console.log("Updating stETH/ETH feed...");
        (bool success2, ) = stethEthFeed.call(
            abi.encodeWithSignature("setLatestAnswer(int256)", stethEthPrice)
        );
        require(success2, "Failed to update stETH/ETH feed");

        console.log("Updating wstETH/USD feed...");
        (bool success3, ) = wstethUsdFeed.call(
            abi.encodeWithSignature("setLatestAnswer(int256)", wstethUsdPrice)
        );
