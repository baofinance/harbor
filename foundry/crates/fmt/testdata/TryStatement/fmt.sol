...
contract TryStatement {
    ...
    function test() external {
        ...
        try unknown.empty() {
            unknown.doSomething();
        } catch Error(string memory) {} catch Panic(uint256) {} catch {
            unknown.handleError();
        }

        try unknown.lookupMultipleValues() returns (uint256, uint256, uint256, uint256, uint256) {} catch Error(
            string memory
        ) {} catch {}
        ...
        // comment1
        try
            /* comment2 */ unknown.lookup() // comment3
        returns (
            uint256 // comment4
        ) {} catch // comment5
        {
        ...
        try unknown.empty() {
            // comment8
            unknown.doSomething();
        } /* comment9 */ catch Error(/* comment10 */ string memory) {
            unknown.handleError();
        } catch Panic(/* comment11 */ uint256) {
            unknown.handleError();
        } catch {}
    }
}
