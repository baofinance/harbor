...
# when this implementation is upgraded
event Upgraded:
...
# If you add new storage variables in future versions, reduce this gap accordingly
# Example: if you add 2 new uint256 variables, change this to __gap: uint256[98]
__gap: uint256[100]

# __self: immutable(address)

# # chisel eval 'uint256(keccak256("eip1967.proxy.implementation")) - 1'
...
#     """
#     @notice Implementation of the ERC-1822 {proxiableUUID} function.
#     It is used to validate the implementation's compatibility when performing an upgrade.
#     @return The implementation slot defined by ERC-1822

#     IMPORTANT: A proxy pointing at a proxiable contract should not be considered proxiable itself,
#     because this risks bricking a proxy that upgrades to it, by delegating to itself until out of gas.
#     Thus it is critical that this function revert if invoked through a proxy.
#     This is guaranteed by the asset against __self`notDelegated` modifier.
#     """
#     assert self == __self, "Unauthorized call context - not delegated"
#     # this slot address ensures that only OpenZeppelin UUPS (or copies thereof) can UUPS proxy this implementation
#     return IMPLEMENTATION_SLOT

# @external
# def upgradeToAndCall(newImplementation: address, data: Bytes[1024]):
#     """
...
#     @dev Only callable through proxy, not directly on implementation
#     @param newImplementation Address of the new implementation
#     @param data Encoded function call to execute on new implementation
#     Emits an {Upgraded} event.
#     """
#     # Ensure this is called through proxy, not directly on implementation
#     assert self != __self, "Unauthorized call context - only proxy"
#     # Only manager can upgrade
#     assert msg.sender == self.manager, "Unauthorized"

#     # Verify new implementation supports UUPS
#     assert IERC1822Proxiable(newImplementation).proxiableUUID() == IMPLEMENTATION_SLOT, "Must upgrade oneself"

#     # Update the implementation slot using inline assembly
#     slot: uint256 = convert(IMPLEMENTATION_SLOT, uint256)
#     implementation: uint256 = convert(newImplementation, uint256)

#     # Store to the EIP-1967 implementation slot using assembly
#     slot_value: uint256 = convert(IMPLEMENTATION_SLOT, uint256)
#     new_impl_value: uint256 = convert(newImplementation, uint256)

#     # Use the assembly opcode approach
#     assembly:
#         sstore(slot_value, new_impl_value)


#     log Upgraded(newImplementation)

#     # Execute the encoded function call if data is provided
#     if len(_data) > 0:
#         success: bool = False
#         response: Bytes[1024] = b""

#         # Use raw_call with delegate call to execute the function on new implementation
...
#         assert success, "Upgrade call failed"
