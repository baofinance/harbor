...
interface GaugeController:
    ...
    def is_gauge(addr: address) -> bool: view
...
def _mint_for(gauge_addr: address, _for: address):
    assert GaugeController(self.controller).is_gauge(gauge_addr), "Gauge not in controller"
    gauge_type: int128 = GaugeController(self.controller).gauge_types(gauge_addr)
...
def mint_many(gauge_addrs: address[8]):
    ...
    """
    for i in range(8):
        if gauge_addrs[i] == empty(address):
            break
