...
contract VotingEscrowHelper is IVotingEscrowHelper {
    ...
    constructor(address _ve) {
        _VE = _ve;
        _START = IVotingEscrow(_ve).point_history(1).ts;
    ...
    function totalSupply(uint256 timestamp) external view override returns (uint256) {
        ...
        if (prevSupply.epoch > 0) {
            ...
            Balance memory nextSupply = _supply[week + 1 weeks];
            uint256 nextEpoch = nextSupply.epoch;
            if (nextEpoch == 0) nextEpoch = IVotingEscrow(_VE).epoch();
            (, point) = _binarySearchSupplyPoint(timestamp, prevSupply.epoch, nextEpoch);
        } else {
            (, point) = _binarySearchSupplyPoint(timestamp, 1, IVotingEscrow(_VE).epoch());
        }
    ...
    function balanceOf(address account, uint256 timestamp) external view override returns (uint256) {
        ...
        uint256 epoch = IVotingEscrow(_VE).user_point_epoch(account);
        if (epoch == 0) return 0;
        IVotingEscrow.Point memory point = IVotingEscrow(_VE).user_point_history(account, 1);
        ...
        if (prevBalance.epoch > 0) {
            ...
            Balance memory nextBalance = _balances[account][week + 1 weeks];
    ...
    function checkpoint(address account, uint256 timestamp) external override {
        ...
        if (timestamp < _START) revert ErrorCheckpointInvalidPastTime();
    ...
    function _checkpoint(address account, uint256 week) internal {
        ...
        if (nowSupply.epoch == 0) {
            Balance memory prevSupply = _supply[week - 1 weeks];
            uint256 epoch;
            IVotingEscrow.Point memory point;
            if (prevSupply.epoch == 0) {
                (epoch, point) = _binarySearchSupplyPoint(week, 1, IVotingEscrow(_VE).epoch());
            } else {
                (epoch, point) = _binarySearchSupplyPoint(week, prevSupply.epoch, IVotingEscrow(_VE).epoch());
            }
        ...
        uint256 userPointEpoch = IVotingEscrow(_VE).user_point_epoch(account);
        if (userPointEpoch == 0) return;

        Balance memory nowBalance = _balances[account][week];
        if (nowBalance.epoch == 0) {
            Balance memory prevBalance = _balances[account][week - 1 weeks];
            uint256 epoch;
            IVotingEscrow.Point memory point;
            if (prevBalance.epoch == 0) {
    ...
    function _binarySearchSupplyPoint(
        ...
        unchecked {
            while (startEpoch < endEpoch) {
                ...
                IVotingEscrow.Point memory p = IVotingEscrow(_VE).point_history(mid);
                if (p.ts <= timestamp) {
                    startEpoch = mid;
                    point = p;
                } else {
        ...
        if (point.ts == 0) {
            point = IVotingEscrow(_VE).point_history(epoch);
        }
    }
    ...
    function _binarySearchBalancePoint(
        ...
        unchecked {
            while (startEpoch < endEpoch) {
                uint256 mid = (startEpoch + endEpoch + 1) / 2;
                IVotingEscrow.Point memory p = IVotingEscrow(_VE).user_point_history(account, mid);
                if (p.ts <= timestamp) {
                    startEpoch = mid;
                    point = p;
                } else {
        ...
        // in case, the `p.ts <= timestamp` never hit in the binary search
        if (point.ts == 0) {
            point = IVotingEscrow(_VE).user_point_history(account, epoch);
        }
    }
    ...
    function _veSupplyAt(IVotingEscrow.Point memory point, uint256 timestamp) internal view returns (uint256) {
        ...
        while (true) {
            ...
            if (ti > timestamp) ti = timestamp;
                ...
                dslope = IVotingEscrow(_VE).slope_changes(ti);
            }
