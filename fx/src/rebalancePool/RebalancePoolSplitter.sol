...
contract RebalancePoolSplitter is Ownable2Step, IFxRebalancePoolSplitter {
    ...
    function getReceivers(
        address _token
    ) external view override returns (address[] memory _receivers, uint256[] memory _ratios) {
    ...
    function registerReceiver(address _token, address _receiver, uint256[] memory _ratios) external onlyOwner {
        EnumerableSet.AddressSet storage _receivers = receivers[_token];
    ...
    /// @param _ratios The new split ratio list.
    function deregisterReceiver(address _token, address _receiver, uint256[] memory _ratios) external onlyOwner {
        EnumerableSet.AddressSet storage _receivers = receivers[_token];
