...
contract ShareableRebalancePool is MultipleRewardCompoundingAccumulator, IFxShareableRebalancePool {
    ...
    function initialize(address _treasury, address _market, address _gauge) external initializer {
    ...
    function liquidate(
        uint256 _maxAmount,
        uint256 _minBaseOut
    ) external virtual override onlyRole(LIQUIDATOR_ROLE) returns (uint256 _liquidated, uint256 _baseOut) {
    ...
    function _getUserPoolShare(
        address _account
    ) internal view virtual override returns (uint112 _previousProd, uint256 _share) {
        TokenBalance memory _balance = _balances[_account];
    ...
    function _withdraw(address _sender, uint256 _amount, address _receiver) internal {
        // @note after checkpoint, the account balances are correct, we can `_balances` safely.
        _checkpoint(_sender);
    ...
    ) internal virtual returns (TokenBalance memory _balance) {
    ...
    function _getBoostRatio(address _account) internal view returns (uint256 _boostRatio) {
        ...
        unchecked {
            for (uint256 i = 0; i < 256; ++i) {
                ...
                (_currentRatio, _nextIndex) = _boostRatioAt(
                    _owner,
                    _balance,
                    _veBalance,
                    _veSupply,
