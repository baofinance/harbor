...
abstract contract TreasuryV2 is AccessControlUpgradeable, IFxTreasuryV2 {
    ...
    function mintFToken(
        uint256 _baseIn,
        address _recipient
    ) external override onlyRole(FX_MARKET_ROLE) returns (uint256 _fTokenOut) {
        FxStableMath.SwapState memory _state = _loadSwapState(Action.MintFToken);
    ...
    function mintXToken(
        uint256 _baseIn,
        address _recipient
    ) external override onlyRole(FX_MARKET_ROLE) returns (uint256 _xTokenOut) {
        FxStableMath.SwapState memory _state = _loadSwapState(Action.MintXToken);
        if (_state.xNav == 0) revert ErrorUnderCollateral();
        if (_state.baseSupply + _baseIn > baseTokenCap) revert ErrorExceedTotalCap();
    ...
    function initializeProtocol(
        uint256 _baseIn
    ) external virtual onlyRole(PROTOCOL_INITIALIZER_ROLE) returns (uint256, uint256) {
