...
contract Treasury is OwnableUpgradeable, IFxTreasury {
    ...
    /// @inheritdoc IFxTreasury
    function getCurrentNav() external view override returns (uint256 _baseNav, uint256 _fNav, uint256 _xNav) {
        StableCoinMath.SwapState memory _state = _loadSwapState(SwapKind.None);
    ...
    ) external view override returns (uint256 _maxBaseIn, uint256 _maxFTokenMintable) {
    ...
    function maxMintableXToken(
        uint256 _newCollateralRatio
    ) external view override returns (uint256 _maxBaseIn, uint256 _maxXTokenMintable) {
        require(_newCollateralRatio > PRECISION, "collateral ratio too small");
    ...
    function maxRedeemableFToken(
        uint256 _newCollateralRatio
    ) external view override returns (uint256 _maxBaseOut, uint256 _maxFTokenRedeemable) {
        require(_newCollateralRatio > PRECISION, "collateral ratio too small");
    ...
    function maxRedeemableXToken(
        uint256 _newCollateralRatio
    ) external view override returns (uint256 _maxBaseOut, uint256 _maxXTokenRedeemable) {
        require(_newCollateralRatio > PRECISION, "collateral ratio too small");
    ...
    function maxLiquidatable(
        uint256 _newCollateralRatio,
        uint256 _incentiveRatio
    ) external view override returns (uint256 _maxBaseOut, uint256 _maxFTokenLiquidatable) {
        require(_newCollateralRatio > PRECISION, "collateral ratio too small");
    ...
    function _loadSwapState(SwapKind _kind) internal view returns (StableCoinMath.SwapState memory _state) {
        ...
        if (_state.baseSupply == 0) {
            ...
            if (_state.xSupply == 0) {
                ...
                _state.xNav = _state.baseSupply.mul(_state.baseNav).sub(_state.fSupply.mul(_state.fNav)).div(
                    _state.xSupply
                );
            }
        }
    ...
    function _fetchTwapPrice(SwapKind _kind) internal view returns (uint256 _price) {
        (bool _isValid, uint256 _safePrice, uint256 _minPrice, uint256 _maxPrice) = IFxPriceOracle(priceOracle)
            .getPrice();
