...
interface IFxTreasury {
    ...
    function getCurrentNav() external view returns (uint256 baseNav, uint256 fNav, uint256 xNav);
    ...
    function maxLiquidatable(
        uint256 newCollateralRatio,
        uint256 incentiveRatio
    ) external view returns (uint256 maxBaseOut, uint256 maxFTokenLiquidatable);
    ...
    function liquidate(uint256 fTokenIn, uint256 incentiveRatio, address owner) external returns (uint256 baseOut);
