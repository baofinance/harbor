...
contract TestStabilityPoolClaimable is TestStabilityPoolRebalanceSetUp {
    ...
    function testClaimSingle_claimsOnlySpecifiedToken() public {
        ...
        vm.prank(user1);
        IStabilityPool_v3(stabilityPoolCollateral).claimSingle(user1, rewardToken1);
        ...
        assertEq(IERC20(rewardToken2).balanceOf(user1), bal2Before, "rewardToken2 untouched");
    ...
    function testClaimSingle_withReceiver() public {
        ...
        vm.prank(user1);
        IStabilityPool_v3(stabilityPoolCollateral).claimSingle(user1, rewardToken1, receiver);
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // Fractional claimSingle tests
    // ═══════════════════════════════════════════════════════════════════════

    function testClaimSingle_fractional_claimsPartial() public {
        _depositForUsers();
        _depositRewardAndWait(rewardToken1, 100 ether);

        uint256 claimable = IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(user1, rewardToken1);
        assertGt(claimable, 0, "should have claimable");

        // Claim half
        uint256 halfAmount = claimable / 2;
        uint256 balBefore = IERC20(rewardToken1).balanceOf(user1);
        vm.prank(user1);
        IStabilityPool_v3(stabilityPoolCollateral).claimSingle(user1, rewardToken1, halfAmount);

        // Received exactly halfAmount
        assertEq(IERC20(rewardToken1).balanceOf(user1) - balBefore, halfAmount, "received half");

        // Remainder still claimable
        uint256 remaining = IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(user1, rewardToken1);
        assertApproxEqAbs(remaining, claimable - halfAmount, 1, "remainder still claimable");
    }

    function testClaimSingle_fractional_claimAll() public {
        _depositForUsers();
        _depositRewardAndWait(rewardToken1, 100 ether);
        ...
        uint256 balBefore = IERC20(rewardToken1).balanceOf(user1);
        vm.prank(user1);
        IStabilityPool_v3(stabilityPoolCollateral).claimSingle(user1, rewardToken1, type(uint256).max);

        assertEq(IERC20(rewardToken1).balanceOf(user1) - balBefore, claimable, "claimed all");

        // Nothing remaining
        uint256 remaining = IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(user1, rewardToken1);
        assertEq(remaining, 0, "nothing remaining");
    }

    function testClaimSingle_fractional_claimZero() public {
        _depositForUsers();
        _depositRewardAndWait(rewardToken1, 100 ether);

        uint256 claimable = IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(user1, rewardToken1);

        // Claim zero — should be a no-op
        uint256 balBefore = IERC20(rewardToken1).balanceOf(user1);
        vm.prank(user1);
        IStabilityPool_v3(stabilityPoolCollateral).claimSingle(user1, rewardToken1, 0);

        assertEq(IERC20(rewardToken1).balanceOf(user1), balBefore, "nothing transferred");
        assertEq(
            IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(user1, rewardToken1),
            claimable,
            "claimable unchanged"
    ...
    function testClaimSingle_fractional_withReceiver() public {
        _depositForUsers();
        _depositRewardAndWait(rewardToken1, 100 ether);
        ...
        address receiver = makeAddr("receiver");
        uint256 partialAmount = claimable / 3;
        vm.prank(user1);
        IStabilityPool_v3(stabilityPoolCollateral).claimSingle(user1, rewardToken1, receiver, partialAmount);

        assertEq(IERC20(rewardToken1).balanceOf(receiver), partialAmount, "receiver got partial");
        assertEq(IERC20(rewardToken1).balanceOf(user1), 0, "user got nothing");

        // Remainder still claimable
        uint256 remaining = IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(user1, rewardToken1);
        assertApproxEqAbs(remaining, claimable - partialAmount, 1, "remainder still claimable");
    }
    ...
    function testClaimSingle_fractional_multipleClaims() public {
        ...
        // Claim in three tranches
        uint256 tranche = claimable / 3;
        vm.startPrank(user1);
        IStabilityPool_v3(stabilityPoolCollateral).claimSingle(user1, rewardToken1, tranche);
        IStabilityPool_v3(stabilityPoolCollateral).claimSingle(user1, rewardToken1, tranche);
        IStabilityPool_v3(stabilityPoolCollateral).claimSingle(user1, rewardToken1, type(uint256).max);
        vm.stopPrank();

        // Should have claimed everything
        assertApproxEqAbs(IERC20(rewardToken1).balanceOf(user1), claimable, 1, "claimed everything in 3 tranches");
        assertEq(
            IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(user1, rewardToken1),
            0,
            "nothing remaining after 3 tranches"
        );
    }

    function testClaimSingle_fractional_linearAccrual_midPeriod() public {
        _depositForUsers();

        // Deposit reward but only wait half the distribution period (604800s = 7 days)
        vm.prank(rewardDepositor);
        IMultipleRewardDistributor(stabilityPoolCollateral).depositReward(rewardToken1, 300 ether);
        skip(3.5 days);

        // ~half should be claimable (distributed linearly)
        uint256 claimable = IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(user1, rewardToken1);
        assertGt(claimable, 0, "mid-period claimable");
        // ~50 ether per user (300/3 users * 50% of period)
        assertApproxEqRel(claimable, 50 ether, 0.02 ether, "~50 per user at midpoint");

        // Fractional claim: take 20 ether
        vm.prank(user1);
        IStabilityPool_v3(stabilityPoolCollateral).claimSingle(user1, rewardToken1, 20 ether);
        assertEq(IERC20(rewardToken1).balanceOf(user1), 20 ether, "received 20");

        // Wait for rest of period
        skip(3.5 days);

        // Full amount now available (minus what was already claimed)
        uint256 finalClaimable = IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(user1, rewardToken1);
        // ~100 per user total, minus 20 already claimed = ~80
        assertApproxEqRel(finalClaimable, 80 ether, 0.02 ether, "~80 remaining after full period");
    ...
    function testClaimSingle_fractional_twoTokens_independent() public {
        _depositForUsers();
        _depositRewardAndWait(rewardToken1, 90 ether);
        _depositRewardAndWait(rewardToken2, 180 ether);

        uint256 claimable1 = IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(user1, rewardToken1);
        uint256 claimable2 = IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(user1, rewardToken2);

        // Partial claim from token1 only
        vm.prank(user1);
        IStabilityPool_v3(stabilityPoolCollateral).claimSingle(user1, rewardToken1, claimable1 / 4);

        // token2 claimable unchanged
        assertEq(
            IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(user1, rewardToken2),
            claimable2,
            "token2 unaffected by token1 partial claim"
        );

        // token1 reduced
        uint256 remaining1 = IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(user1, rewardToken1);
        assertApproxEqAbs(remaining1, claimable1 - claimable1 / 4, 1, "token1 reduced by claimed amount");
    }

    function testClaimSingle_fractional_withReceiver_revertsForOthers() public {
        _depositForUsers();
        _depositRewardAndWait(rewardToken1, 100 ether);

        // user2 tries to claim user1's reward to a custom receiver — should revert
        address receiver = makeAddr("receiver");
        vm.prank(user2);
        vm.expectRevert();
        IStabilityPool_v3(stabilityPoolCollateral).claimSingle(user1, rewardToken1, receiver, 50 ether);
    }
}
...
contract TestRewardAlias is TestStabilityPoolRebalanceSetUp {
    ...
    function setUp() public override {
        ...
        // Register underlying with both aliases (drain order: harvest first, then boost)
        address[] memory aliases = new address[](2);
        aliases[0] = address(harvestAlias);
        aliases[1] = address(boostAlias);
        vm.prank(rewardManager);
        LinearMultipleRewardDistributor_v3(stabilityPoolCollateral).registerRewardToken(
    ...
    // ── Fractional claim with aliases ──────────────────────────────────

    function testAlias_fractionalClaim_partialFromAlias() public {
        _depositRewardAndWait(address(harvestAlias), 100 ether);

        uint256 claimable = IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(user1, address(harvestAlias));
        assertGt(claimable, 0, "should have claimable via alias");

        // Partial claim via alias — should receive underlying token
        uint256 half = claimable / 2;
        uint256 balBefore = IERC20(address(aliasUnderlying)).balanceOf(user1);
        vm.prank(user1);
        IStabilityPool_v3(stabilityPoolCollateral).claimSingle(user1, address(harvestAlias), half);

        assertEq(IERC20(address(aliasUnderlying)).balanceOf(user1) - balBefore, half, "received underlying");

        // Remainder still claimable via alias
        uint256 remaining = IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(user1, address(harvestAlias));
        assertApproxEqAbs(remaining, claimable - half, 1, "remainder via alias");
    }

    function testAlias_fractionalClaim_partialFromOneAlias_otherUnaffected() public {
        _depositRewardAndWait(address(harvestAlias), 60 ether);
        _depositRewardAndWait(address(boostAlias), 40 ether);

        uint256 claimableHarvest = IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(
            user1,
            address(harvestAlias)
        );
        uint256 claimableBoost = IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(
            user1,
            address(boostAlias)
        );
        ...
        vm.prank(user1);
        IStabilityPool_v3(stabilityPoolCollateral).claimSingle(user1, address(harvestAlias), claimableHarvest / 3);

        // boostAlias claimable unchanged
        assertEq(
            IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(user1, address(boostAlias)),
            claimableBoost,
            "boost alias unaffected"
        );

        // harvestAlias reduced
        uint256 remainHarvest = IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(
            user1,
            address(harvestAlias)
        );
        assertApproxEqAbs(remainHarvest, claimableHarvest - claimableHarvest / 3, 1, "harvest alias reduced");
    }

    function testAlias_fractionalClaim_thenClaimUnderlying_aggregated() public {
        _depositRewardAndWait(address(harvestAlias), 60 ether);
        _depositRewardAndWait(address(boostAlias), 40 ether);

        // Aggregated claimable for underlying = sum of both aliases
        uint256 aggregated = IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(
            user1,
            address(aliasUnderlying)
        );
        ...
        assertApproxEqAbs(aggregated, claimableHarvest + claimableBoost, 2, "aggregated = sum of aliases");

        // Partial claim from harvestAlias
        vm.prank(user1);
        IStabilityPool_v3(stabilityPoolCollateral).claimSingle(user1, address(harvestAlias), claimableHarvest / 2);
        ...
        assertApproxEqAbs(newAggregated, aggregated - claimableHarvest / 2, 2, "aggregated reduced by partial claim");
    }

    function testAlias_fractionalClaim_withReceiver() public {
        _depositRewardAndWait(address(harvestAlias), 100 ether);

        uint256 claimable = IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(user1, address(harvestAlias));
        address receiver = makeAddr("aliasReceiver");
        uint256 partialAmount = claimable / 4;

        vm.prank(user1);
        IStabilityPool_v3(stabilityPoolCollateral).claimSingle(user1, address(harvestAlias), receiver, partialAmount);
        ...
        assertEq(IERC20(address(aliasUnderlying)).balanceOf(user1), 0, "user got nothing");

        // Remainder
        uint256 remaining = IMultipleRewardAccumulator(stabilityPoolCollateral).claimable(user1, address(harvestAlias));
        assertApproxEqAbs(remaining, claimable - partialAmount, 1, "remainder after partial alias claim");
    }
}
